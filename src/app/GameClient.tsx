...
export function GameClient({ initialDate }: { initialDate: string }) {
  // initialDate comes from the server at request time — always correct.
  // The midnight timeout swaps the bird without a page reload.
  ...
  const handleReveal = (squareIndex: number) => {
    setHighlighted(null);
    setFeedback(null);
    // Use functional update so we always get the latest revealed array,
    // not the one captured by this closure at render time.
    setRevealed((prev) => {
      const nextRevealed = [...prev, squareIndex];
